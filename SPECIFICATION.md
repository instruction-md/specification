...
auth: { kind: static, token: "{{secret:TICKETING_TOKEN}}" }
:::
```
...
|---|---|---|---|---|---|
| `!agent` | C L | `name`, `template`, `ttl` | YAML: params | `compose` | `[subagent "x" is available]` |
...
comments (`←`) are annotations, not part of the document.
...
`::!channel` lines; a fenced tool policy with its prose; a fenced server
